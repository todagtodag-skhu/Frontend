...
type CompletedStickerBoardsScreenProps = {
  boards: CompletedStickerBoard[];
  isLoading: boolean;
  error?: Error | null;
  onSwipeBack?: () => void;
};
...
export function CompletedStickerBoardsScreen({
  boards,
  isLoading,
  error,
  onSwipeBack,
}: CompletedStickerBoardsScreenProps) {
    ...
    () =>
      PanResponder.create({
        ...
        onPanResponderRelease: (_, gestureState) => {
          if (onSwipeBack && gestureState.dx >= 60) {
            onSwipeBack();
          }
        },
      }),
